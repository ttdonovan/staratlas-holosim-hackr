...
async fn dump_program_accounts(
    ...
    for program_id in program_ids {
        ...
        info!("Found {} accounts for program {}", accounts.len(), program_id);
        
        let account_count = accounts.len();
        
        // Convert accounts to AccountData and batch insert
        let account_data_vec: Vec<AccountData> = accounts.into_iter().map(|(account_pubkey, account)| {
            AccountData {
        ...
        if let Err(e) = database.batch_upsert_accounts(&account_data_vec).await {
            error!("Failed to batch insert accounts for program {}: {}", program_id, e);
        }
        
        info!("✅ Completed dumping {} accounts for program {}", account_count, program_id);
    }
