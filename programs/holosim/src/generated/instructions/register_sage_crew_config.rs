...
pub const REGISTER_SAGE_CREW_CONFIG_DISCRIMINATOR: [u8; 8] = [221, 144, 125, 158, 28, 25, 186, 209];
...
impl RegisterSageCrewConfig {
    pub fn instruction(
        &self,
        args: RegisterSageCrewConfigInstructionArgs,
    ...
    pub fn instruction_with_remaining_accounts(
        ...
        let mut data = borsh::to_vec(&RegisterSageCrewConfigInstructionData::new()).unwrap();
...
pub struct RegisterSageCrewConfigInstructionData {
    discriminator: [u8; 8],
}

impl RegisterSageCrewConfigInstructionData {
    pub fn new() -> Self {
        Self {
            discriminator: [221, 144, 125, 158, 28, 25, 186, 209],
        }
    }
}

impl Default for RegisterSageCrewConfigInstructionData {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(BorshSerialize, BorshDeserialize, Clone, Debug, Eq, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct RegisterSageCrewConfigInstructionArgs {
    pub input: KeyIndexInput,
}

/// Instruction builder for `RegisterSageCrewConfig`.
///
/// ### Accounts:
///
///   0. `[writable, signer]` funder
///   1. `[writable]` sage_crew_config
///   2. `[]` config
///   3. `[signer]` key
///   4. `[]` profile
///   5. `[]` game_id
...
pub struct RegisterSageCrewConfigBuilder {
...
impl RegisterSageCrewConfigBuilder {
    pub fn new() -> Self {
        Self::default()
    }
    /// The funder for the new star base
    ...
    pub fn instruction(&self) -> solana_instruction::Instruction {
        let accounts = RegisterSageCrewConfig {
        ...
        let args = RegisterSageCrewConfigInstructionArgs {
...
/// `register_sage_crew_config` CPI accounts.
pub struct RegisterSageCrewConfigCpiAccounts<'a, 'b> {
    /// The funder for the new star base
...
/// `register_sage_crew_config` CPI instruction.
pub struct RegisterSageCrewConfigCpi<'a, 'b> {
    ...
    pub __args: RegisterSageCrewConfigInstructionArgs,
}

impl<'a, 'b> RegisterSageCrewConfigCpi<'a, 'b> {
    pub fn new(
        program: &'b solana_account_info::AccountInfo<'a>,
        accounts: RegisterSageCrewConfigCpiAccounts<'a, 'b>,
        args: RegisterSageCrewConfigInstructionArgs,
    ) -> Self {
        Self {
            __program: program,
...
/// Instruction builder for `RegisterSageCrewConfig` via CPI.
///
/// ### Accounts:
///
///   0. `[writable, signer]` funder
...
///   6. `[]` system_program
#[derive(Clone, Debug)]
pub struct RegisterSageCrewConfigCpiBuilder<'a, 'b> {
    instruction: Box<RegisterSageCrewConfigCpiBuilderInstruction<'a, 'b>>,
}

impl<'a, 'b> RegisterSageCrewConfigCpiBuilder<'a, 'b> {
    pub fn new(program: &'b solana_account_info::AccountInfo<'a>) -> Self {
        let instruction = Box::new(RegisterSageCrewConfigCpiBuilderInstruction {
            __program: program,
            funder: None,
    ...
    pub fn invoke_signed(&self, signers_seeds: &[&[&[u8]]]) -> solana_program_error::ProgramResult {
        ...
        let instruction = RegisterSageCrewConfigCpi {
...
#[derive(Clone, Debug)]
struct RegisterSageCrewConfigCpiBuilderInstruction<'a, 'b> {
