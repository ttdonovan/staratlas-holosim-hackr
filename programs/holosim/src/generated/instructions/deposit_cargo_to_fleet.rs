...
pub const DEPOSIT_CARGO_TO_FLEET_DISCRIMINATOR: [u8; 8] = [55, 235, 105, 123, 0, 253, 64, 237];
...
impl DepositCargoToFleet {
    pub fn instruction(
        &self,
        args: DepositCargoToFleetInstructionArgs,
    ...
    pub fn instruction_with_remaining_accounts(
        ...
        let mut data = borsh::to_vec(&DepositCargoToFleetInstructionData::new()).unwrap();
...
pub struct DepositCargoToFleetInstructionData {
    discriminator: [u8; 8],
}

impl DepositCargoToFleetInstructionData {
    pub fn new() -> Self {
        Self {
            discriminator: [55, 235, 105, 123, 0, 253, 64, 237],
        }
    }
}

impl Default for DepositCargoToFleetInstructionData {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(BorshSerialize, BorshDeserialize, Clone, Debug, Eq, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct DepositCargoToFleetInstructionArgs {
    pub amount: u64,
    pub key_index: u16,
}

/// Instruction builder for `DepositCargoToFleet`.
///
/// ### Accounts:
///
///   0. `[signer]` key
...
///   7. `[]` starbase
///   8. `[]` starbase_player
///   9. `[writable]` cargo_pod_from
///   10. `[writable]` cargo_pod_to
///   11. `[]` cargo_type
///   12. `[]` cargo_stats_definition
///   13. `[writable]` token_from
///   14. `[writable]` token_to
///   15. `[writable]` token_mint
///   16. `[]` cargo_program
///   17. `[optional]` token_program (default to `TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA`)
#[derive(Clone, Debug, Default)]
pub struct DepositCargoToFleetBuilder {
...
impl DepositCargoToFleetBuilder {
    pub fn new() -> Self {
        Self::default()
    }
    /// The key on the profile.
    ...
    pub fn instruction(&self) -> solana_instruction::Instruction {
        let accounts = DepositCargoToFleet {
        ...
        let args = DepositCargoToFleetInstructionArgs {
...
/// `deposit_cargo_to_fleet` CPI accounts.
pub struct DepositCargoToFleetCpiAccounts<'a, 'b> {
    /// The key on the profile.
...
/// `deposit_cargo_to_fleet` CPI instruction.
pub struct DepositCargoToFleetCpi<'a, 'b> {
    ...
    pub __args: DepositCargoToFleetInstructionArgs,
}

impl<'a, 'b> DepositCargoToFleetCpi<'a, 'b> {
    pub fn new(
        program: &'b solana_account_info::AccountInfo<'a>,
        accounts: DepositCargoToFleetCpiAccounts<'a, 'b>,
        args: DepositCargoToFleetInstructionArgs,
    ) -> Self {
        Self {
            __program: program,
...
/// Instruction builder for `DepositCargoToFleet` via CPI.
///
/// ### Accounts:
///
///   0. `[signer]` key
...
///   17. `[]` token_program
#[derive(Clone, Debug)]
pub struct DepositCargoToFleetCpiBuilder<'a, 'b> {
    instruction: Box<DepositCargoToFleetCpiBuilderInstruction<'a, 'b>>,
}

impl<'a, 'b> DepositCargoToFleetCpiBuilder<'a, 'b> {
    pub fn new(program: &'b solana_account_info::AccountInfo<'a>) -> Self {
        let instruction = Box::new(DepositCargoToFleetCpiBuilderInstruction {
            __program: program,
            key: None,
            owning_profile: None,
    ...
    pub fn invoke_signed(&self, signers_seeds: &[&[&[u8]]]) -> solana_program_error::ProgramResult {
        ...
        let instruction = DepositCargoToFleetCpi {
...
#[derive(Clone, Debug)]
struct DepositCargoToFleetCpiBuilderInstruction<'a, 'b> {
